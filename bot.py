...
def stake_token(w3, account, private_key, token):
    ...
    if token == "ausd":
        ...
        raw_balance = int(get_erc20_balance(w3, c["token_contract"], c["decimals"], address) * (10 ** c["decimals"]))
    if raw_balance == 0:
...
if __name__ == "__main__":
    print("=== MAITRIX BOT (Auto Mint + Stake) ===")
    if not PRIVATE_KEYS:
        print("[!] Tidak ada PRIVATE_KEY terdeteksi di .env")
